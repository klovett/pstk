    ...
    (else
      (let loop ((char-list '()))
        (if (eof-object? (peek-char input-port))
          ...
          (loop (cons (read-char input-port) char-list)))) ) ) )

;;

(define-values (tk $) (pstk))
        ...
        (let ((filename ($ tk/get-open-file)))
          (unless (string-null? filename)
            ...
            (call-with-input-file
              filename
              (lambda (input-port)
                (text 'insert 1.0 (get-string-all input-port))))))))
      ...
      command: (lambda ()
        (let ((filename ($ tk/get-save-file)))
          (unless (string-null? filename)
            (with-output-to-file
              filename
              (lambda () (display (text 'get 1.0 "end"))))))))
    ;
    (tk 'configure menu: menubar) )
  ;
  (let ((vsb (tk 'create-widget 'scrollbar orient: 'vertical)))
